...
def count_fingers(thresholded_image):
    if hand is None:
        return 0
    width_range = abs(hand.right[0] - hand.left[0])
    if width_range == 0:
        return 0
    ...
    for curr in contours:
        width = len(curr)
        if width < 3 * width_range / 4 and width > 5:
            fingers += 1
    return fingers
...
def most_frequent(input_list):
    ...
    for item in reversed(input_list):
        ...
        if count_dict[item] >= count:
            count, most_freq = count_dict[item], item
    return most_freq
...
def write_on_image(frame):
    ...
    if frames_elapsed < CALIBRATION_TIME:
        ...
        if hand.isWaving:
            ...
            text = "Danger"
    cv2.putText(frame, text, (10, 20), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0), 2)
    cv2.putText(frame, text, (10, 20), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 1)
...
while True:
    ...
    if frames_elapsed < CALIBRATION_TIME:
        ...
        if region_pair:
            thresholded_image, segmented_image = region_pair
            get_hand_data(thresholded_image, segmented_image)
            if hand:
